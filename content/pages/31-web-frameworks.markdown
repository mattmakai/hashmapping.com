...
meta: Web frameworks are code libraries that provide patterns to solve common web application development challenges.


# Web frameworks
Web frameworks, also commonly referred to as "web application frameworks", 
are code libraries which make it easier for software developers to create 
and maintain websites and web applications. 

Web frameworks are a conceptual idea and are implemented in many programming 
languages. For example, Python's web framework implementations include 
Django, Flask and Bottle (among others) while Java's web frameworks are
Google Web Toolkit, Spark and Struts.
